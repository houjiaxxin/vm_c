...
#include <cstdint>
#include <cstring>
// vm_c 兼容层：GCC 7.3.0 libstdc++ cstring 头不传递性 declare 全局
// ::memset/memcpy/memmove。显式 include C 头以确保这些符号在 <cstring> 之后可用。
#include <string.h>
#include <future>
#include <regex>
...
struct ggml_tensor * llama_model_loader::create_tensor(
    ...
    if (files.empty()) {
        ...
        ggml_tensor t_meta{};   // C++ value init 替代 memset（兼容 GCC 7.3.0 cstring 头传递性不足）
        t_meta.type = type;
...
void llama_model_loader::done_getting_tensors(bool partial) const {
    ...
    if (n_created < n_tensors) {
        ...
        LLAMA_LOG_INFO("%s: registered %d model tensors from GGUF\n", __func__, n_created);
    }
    if (n_tensors_moved > 0) {
        LLAMA_LOG_INFO("%s: %zu tensor(s) use fallback buffer (example '%s' %s): %s instead of %s\n",
            __func__, n_tensors_moved, first_tensor_moved_name.c_str(), first_tensor_moved_type_name.c_str(),
            ggml_backend_buft_name(first_moved_to_buft), ggml_backend_buft_name(first_moved_from_buft));
    }
}

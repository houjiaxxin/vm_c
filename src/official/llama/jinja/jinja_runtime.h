...
#include <cassert>
#include <cstdio>   // vm_c 补充：JJ_DEBUG 宏用 printf，旧版 libstdc++ 需显式 include
#include <ctime>
#include <memory>
#include <sstream>
#include <string>
...
struct program : public statement {
    ...
    // vm_c 补充：base class statement 有 user-declared virtual dtor，会抑制 implicit
    // move ctor/assign 的生成，导致 std::make_unique<program>(prvalue) 等用法失败。
    // 显式 = default 让编译器生成 move 成员（move base + move body）。
    program(program && other) noexcept = default;
    program & operator=(program && other) noexcept = default;
    // 拷贝构造因 body 含 unique_ptr 不可拷贝

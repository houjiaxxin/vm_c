...
llama_context::llama_context(
    ...
    cparams.vm_c_kv_quant_method = params.vm_c_kv_quant_method;
    cparams.vm_c_tq_bridge       = params.vm_c_tq_bridge;
    cparams.vm_c_tp              = params.vm_c_tp;
    cparams.vm_c_tp_size         = params.vm_c_tp_size;
#endif
...
void llama_context::sched_reserve() {
    ...
    // reserve again with pp graph to avoid ggml-alloc reallocations during inference
    {
        ...
        //
        // no_alloc 仅表示权重由外部 rebind，compute buffer 仍须在此步 materialize；
        // 若 split_only=true（旧逻辑误用 hparams.no_alloc），galloc->buffers 保持 NULL，
        // 首次 decode 的 ggml_gallocr_alloc_graph 会在 ggml_vbuffer_tensor_alloc 处 SIGSEGV。
        auto * gf = graph_reserve(n_tokens, n_seqs, n_tokens, mctx.get(), /*split_only =*/ false);
        if (!gf) {
            throw std::runtime_error("failed to allocate compute pp buffers");
        }
    }
...
llm_graph_result * llama_context::process_ubatch(const llama_ubatch & ubatch, llm_graph_type gtype, llama_memory_context_i * mctx, ggml_status & ret) {
    ...
    // vm_c 优化：process_ubatch 每步打印会严重刷屏（每 token 一次）。
    // 改为 DEBUG 级（默认不输出）；保留 4 个 DEBUG 环境变量开关：
    //   VMC_LOG_PROCESS_UBATCH=1  打印每步（默认 0）
    //   VMC_LOG_PROCESS_UBATCH_EVERY=N  每 N 步打印一次
    //   VMC_LOG_PROCESS_UBATCH_PREFILL=1  仅 prefill（n_tokens>1 且非 verify）打印
    //   VMC_LOG_PROCESS_UBATCH_VERIFY=1  仅 MTP verify（n_tokens=1+5draft）打印
    static const char* env_always = getenv("VMC_LOG_PROCESS_UBATCH");
    static const char* env_every  = getenv("VMC_LOG_PROCESS_UBATCH_EVERY");
    static const char* env_prefill = getenv("VMC_LOG_PROCESS_UBATCH_PREFILL");
    static const char* env_verify = getenv("VMC_LOG_PROCESS_UBATCH_VERIFY");
    static thread_local int step_counter = 0;
    ++step_counter;
    bool log_now = false;
    if (env_always && env_always[0] == '1') {
        log_now = true;
    } else if (env_every && atoi(env_every) > 0 && (step_counter % atoi(env_every)) == 0) {
        log_now = true;
    } else if (env_prefill && env_prefill[0] == '1' && ubatch.n_tokens > 1
               && cparams.ctx_type != 0 /* skip verify */) {
        // prefill = 一次性处理整个 prompt（>1 token）
        log_now = true;
    } else if (env_verify && env_verify[0] == '1' && cparams.ctx_type == 0
               && ubatch.n_tokens == 6 /* n_logits_rows=6 in MTP verify */) {
        log_now = true;
    }
    if (log_now) {
        LLAMA_LOG_INFO("[DEBUG] process_ubatch: step=%d ctx_type=%d n_tokens=%u ubatch.token=%p ubatch.embd=%p n_outputs=%d\n",
            step_counter, (int)cparams.ctx_type, ubatch.n_tokens, (void*)ubatch.token, (void*)ubatch.embd, gparams.n_outputs);
    }
    ...
    if (!graph_reuse_disable && res->can_reuse(gparams)) {
        ...
        // vm_c 诊断：graph 重建时清空 hidden state 探针列表
        vm_c::hidden_log_clear();
    ...
    // vm_c 诊断：graph compute 完成后 dump 已注册的 hidden state 探针
    vm_c::hidden_log_dump_all(sched.get());
...
int llama_context::decode(const llama_batch & batch_inp) {
    ...
    do {
        ...
        // [vm_c] logits 摘要：默认只在 MTP verify 路径打（每 step 1 行）。
        // 主 prefill 路径默认不打开（已确认 valid），避免数千行输出。
        //   VMC_LOG_LOGITS=1          打开全部 ctx_type
        //   VMC_LOG_LOGITS_VERIFY=1   仅 MTP verify 路径（默认行为）
        // 异常（top1=0 或 top5 含 -inf）时升级为 WARN。
#ifdef VM_C_LLAMA_INTEGRATION
        {
            static const char* env_logits      = getenv("VMC_LOG_LOGITS");
            static const char* env_logits_v    = getenv("VMC_LOG_LOGITS_VERIFY");
            const bool enable_all  = env_logits && env_logits[0] == '1';
            const bool enable_mtp  = env_logits_v && env_logits_v[0] == '1';
            const bool is_mtp_path = cparams.ctx_type != 0;
            const bool should_log  = n_outputs > 0 && (enable_all || (enable_mtp && is_mtp_path));
            if (should_log) {
                std::vector<float> host_logits((size_t)n_outputs * (size_t)n_vocab);
                ggml_backend_tensor_get_async(backend_res, t_logits,
                                              host_logits.data(), 0,
                                              host_logits.size() * sizeof(float));
                ggml_backend_sched_synchronize(sched.get());
                const float* row = host_logits.data() + (size_t)(n_outputs - 1) * (size_t)n_vocab;
                ...
                for (int64_t v = 0; v < n_vocab; ++v) {
                    ...
                    if (f > topv[0]) {
                        topv[4] = topv[3]; topk[4] = topk[3];
                        topv[3] = topv[2]; topk[3] = topk[2];
                        topv[2] = topv[1]; topk[2] = topk[1];
                        topv[1] = topv[0]; topk[1] = topk[0];
                        topv[0] = f;        topk[0] = (int)v;
                    }
                }
                const float logits_rms = std::sqrt(sum_sq / n_vocab);
                const bool abnormal = (topk[0] == 0) || std::isinf(topv[0]) || std::isnan(logits_rms);
                if (abnormal) {
                    LLAMA_LOG_WARN("[LOGITS-SUMMARY] ABNORMAL ctx_type=%d n_outputs=%d top1_id=%d top1_val=%.2f rms=%.3f top5=[%d,%d,%d,%d,%d] vals=[%.2f,%.2f,%.2f,%.2f,%.2f]\n",
                                   (int)cparams.ctx_type, n_outputs,
                                   topk[0], topv[0], logits_rms,
                                   topk[0], topk[1], topk[2], topk[3], topk[4],
                                   topv[0], topv[1], topv[2], topv[3], topv[4]);
                } else {
                    LLAMA_LOG_INFO("[LOGITS-SUMMARY] ctx_type=%d n_outputs=%d top1_id=%d top1_val=%.2f rms=%.3f\n",
                                   (int)cparams.ctx_type, n_outputs, topk[0], topv[0], logits_rms);
                }
            }
        }
#endif
...
uint32_t llama_context::output_reserve(int32_t n_outputs) {
    ...
    const size_t new_size  =
        (logits.size + embd.size + embd_pre_norm.size) * sizeof(float);
...
ggml_cgraph * llama_context::graph_reserve(
    ...
    if (cparams.ctx_type == LLAMA_CONTEXT_TYPE_MTP) {
        ubatch.data->embd.resize(static_cast<size_t>(n_tokens) * static_cast<size_t>(model.hparams.n_embd));
...
#ifdef VM_C_LLAMA_INTEGRATION
        /*.vm_c_kv_quant_method        =*/ nullptr,
        /*.vm_c_tq_bridge              =*/ nullptr,
        /*.vm_c_tp                     =*/ nullptr,
        /*.vm_c_tp_size                =*/ 1,
#endif
...
void llama_set_tp_context(llama_context * ctx, void * tp_comm, int32_t tp_size) {
    ctx->set_tp_context(tp_comm, tp_size);
}

...
// vm_c: sampler stubs — vm_c 使用自己的 sampler 管线，不依赖 llama_sampler_chain。
// 这些 stub 仅用于链接，因 llama-context.cpp 中仍有少量校验/日志引用。
// llama-sampler.cpp（独立上层模块）已在融合时移除。

struct llama_sampler * llama_sampler_chain_get(struct llama_sampler * chain, int32_t /*i*/) {
    // 按 llama.h 约定：i == -1 返回 chain 自身（用于检查是否 chain 类型）
    return chain;
}

...
llama_kv_cache::llama_kv_cache(
    ...
    const  layer_reuse_cb & reuse,
                 int32_t   vm_c_tp_size) :
    model(model), hparams(model.hparams), v_trans(v_trans),
    n_seq_max(n_seq_max), n_stream(unified ? 1 : n_seq_max), vm_c_tp_size_(vm_c_tp_size > 1 ? vm_c_tp_size : 1),
    ...
    for (uint32_t il = 0; il < hparams.n_layer; il++) {
        ...
        // [TAG_V_CACHE_VARIABLE]
        const uint32_t n_embd_k_gqa =            n_embd_k_gqa_tp(il);
        const uint32_t n_embd_v_gqa = !v_trans ? n_embd_v_gqa_tp(il) : hparams.n_embd_v_gqa_max() / vm_c_tp_size_;
    ...
    LLAMA_LOG_INFO("%s: KV cache: %d layers active, %d without KV, %d filtered (n_layer=%u)\n",
        __func__, kv_layers, kv_no_cache, kv_filtered, hparams.n_layer);
    ...
    if (reuse) {
        ...
        if (reuse_mapped > 0) {
            LLAMA_LOG_INFO("%s: KV layer reuse map: %d layers\n", __func__, reuse_mapped);
        }
    }
...
uint32_t llama_kv_cache::get_n_layers() const {
    return (uint32_t) layers.size();
}

uint32_t llama_kv_cache::n_head_kv_tp(uint32_t il) const {
    const uint32_t full = hparams.n_head_kv(il);
    return vm_c_tp_size_ > 1 ? full / vm_c_tp_size_ : full;
}

uint32_t llama_kv_cache::n_embd_k_gqa_tp(uint32_t il) const {
    return hparams.n_embd_head_k(il) * n_head_kv_tp(il);
}

uint32_t llama_kv_cache::n_embd_v_gqa_tp(uint32_t il) const {
    return hparams.n_embd_head_v(il) * n_head_kv_tp(il);
}
...
ggml_tensor * llama_kv_cache::get_k(ggml_context * ctx, int32_t il, uint32_t n_kv, const slot_info & sinfo) const {
    ...
    assert(n_embd_k_gqa == n_embd_k_gqa_tp(il));

    const uint32_t ns = sinfo.s1 - sinfo.s0 + 1;

    return ggml_view_4d(ctx, k,
            hparams.n_embd_head_k(il), n_head_kv_tp(il), n_kv, ns,
...
ggml_tensor * llama_kv_cache::get_v(ggml_context * ctx, int32_t il, uint32_t n_kv, const slot_info & sinfo) const {
    ...
    // [TAG_V_CACHE_VARIABLE]
    assert(n_embd_v_gqa >= n_embd_v_gqa_tp(il));
    ...
    if (!v_trans) {
        ...
        return ggml_view_4d(ctx, v,
                hparams.n_embd_head_v(il), n_head_kv_tp(il), n_kv, ns,
    ...
    // note: v->nb[1] > v->nb[2]
    return ggml_view_4d(ctx, v,
            n_kv, n_head_kv_tp(il), hparams.n_embd_head_v(il), ns,
...
ggml_tensor * llama_kv_cache::build_input_v_idxs(ggml_context * ctx, const llama_ubatch & ubatch) const {
    ...
    if (!v_trans) {
        ...
        v_idxs = ggml_new_tensor_1d(ctx, GGML_TYPE_I64, n_tokens*(hparams.n_embd_v_gqa_max() / vm_c_tp_size_));
    }
...
void llama_kv_cache::set_input_v_idxs(ggml_tensor * dst, const llama_ubatch * ubatch, const slot_info & sinfo) const {
    ...
    if (!v_trans) {
        ...
        const int64_t n_embd_v_gqa = hparams.n_embd_v_gqa_max() / vm_c_tp_size_;
...
ggml_cgraph * llama_kv_cache::build_graph_shift(llm_graph_result * res, llama_context * lctx) const {
    ...
    for (const auto & layer : layers) {
        ...
        const int64_t n_head_kv    = n_head_kv_tp(il);
        const int64_t n_embd_k_gqa = n_embd_k_gqa_tp(il);
...
void llama_kv_cache::state_write_data(llama_io_write_i & io, const cell_ranges_t & cr) const {
    ...
    if (!v_trans) {
        for (const auto & layer : layers) {
            ...
            const uint32_t n_embd_v_gqa = n_embd_v_gqa_tp(il);
...
uint32_t llama_kv_cache_context::get_n_layers() const {
    return kv ? kv->get_n_layers() : 0;
}

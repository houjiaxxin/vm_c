...
llama_memory_recurrent::llama_memory_recurrent(
    ...
    const layer_filter_cb & filter,
                 int32_t   vm_c_tp_size) : hparams(model.hparams), n_seq_max(n_seq_max) {
#ifdef VM_C_LLAMA_INTEGRATION
    const int tp = (vm_c_tp_size > 1) ? vm_c_tp_size : 1;
    n_embd_r_row = static_cast<uint32_t>(vm_c_tp_local_n_embd_r(
        hparams.ssm_d_conv, hparams.ssm_d_inner, hparams.ssm_n_group, hparams.ssm_d_state, tp));
    n_embd_s_row = static_cast<uint32_t>(vm_c_tp_local_n_embd_s(
        hparams.ssm_d_state, hparams.ssm_d_inner, hparams.ssm_dt_rank, tp));
#else
    (void) vm_c_tp_size;
    n_embd_r_row = hparams.n_embd_r();
    n_embd_s_row = hparams.n_embd_s();
#endif
    ...
    for (int i = 0; i < n_layer; i++) {
        ...
        ggml_tensor * s = ggml_new_tensor_2d(ctx, type_s, n_embd_s_row, n_rows);
    ...
    LLAMA_LOG_INFO("%s: allocated recurrent state for %d layers (%d skipped)\n",
        __func__, rs_layers, rs_skipped);
...
void llama_memory_recurrent::state_write_data(llama_io_write_i & io, const std::vector<std::pair<uint32_t, uint32_t>> & cell_ranges) const {
    ...
    if (!s_trans) {
        for (uint32_t il = 0; il < n_layer; ++il) {
            ...
            const uint64_t s_size_row = ggml_row_size(s_l[il]->type, n_embd_s_row);
        ...
        for (uint32_t il = 0; il < n_layer; ++il) {
            ...
            const uint32_t n_embd_s = n_embd_s_row;
...
bool llama_memory_recurrent::state_read_data(llama_io_read_i & io, uint32_t cell_count) {
    ...
    if (!s_trans) {
        for (uint32_t il = 0; il < n_layer; ++il) {
            ...
            const size_t s_size_row = ggml_row_size(s_l[il]->type, n_embd_s_row);

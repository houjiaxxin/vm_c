...
extern "C" {
    ...
    struct llama_context_params {
        ...
        const char * vm_c_kv_quant_method;
        void *         vm_c_tq_bridge;
        void *         vm_c_tp;
    ...
    // [vm_c] 单个 attention head 的 key/value 维度（来自 GGUF attention.key_length /
    // attention.value_length）。注意：n_head = n_embd / n_embd_head_k，因此
    // 不能用 n_embd / n_head 反推（Qwen3.5-A3B: 2048/16=128，错误；实际 256）。
...
#define LLAMA_STATE_SEQ_FLAGS_ON_DEVICE 2
    ...
    // Set tensor parallelism context
    // Called before graph building to enable allreduce nodes in the compute graph
    LLAMA_API void llama_set_tp_context(struct llama_context * ctx, void * tp_comm, int32_t tp_size);

...
llama_model_qwen35moe::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    vm_c::hidden_log_register("main.h_post_embed", inpL);
    ...
    for (int il = 0; il < n_transformer_layers; ++il) {
        ...
        cb(cur, "l_out", il);
        // vm_c 诊断：每隔 8 层记一次 hidden state（最多 ~6 次），避免刷屏
        if (il == 0 || il == 8 || il == 16 || il == 24 || il == 32 || il == 40) {
            char name[64];
            std::snprintf(name, sizeof(name), "main.h_post_layer_%d", il);
            vm_c::hidden_log_register(name, cur);
        }
    ...
    vm_c::hidden_log_register("main.h_pre_norm_last", cur);
    ...
    res->t_embd = cur;
    vm_c::hidden_log_register("main.h_post_final_norm", cur);
    ...
    res->t_logits = cur;
    vm_c::hidden_log_register("main.logits_post_allreduce", cur);
...
ggml_tensor * llama_model_qwen35moe::graph::build_layer_attn(
    ...
    // [vm_c] build_layer_attn 入口：标记此层走 MHA 路径（与 recurrent 对比）
    // 这里的 build_attn() 会根据 cparams.vm_c_kv_quant_method + bridge 决定
    //   - 走 TurboQuant ggml_vm_c_turboquant_attn()（性能路径，cache=TurboQuant）
    //   - 还是走标准 build_attn_mha()（要求标准 llama_kv_cache 有该层）
    cb(cur, "attn_in", il);

#ifdef VM_C_LLAMA_INTEGRATION
    // TP-sharded head counts: weights are column-split, so each GPU has n_head/tp heads
    const int vm_c_tp = (cparams.vm_c_tp && cparams.vm_c_tp_size > 1) ? cparams.vm_c_tp_size : 1;
    const int64_t n_head_local    = vm_c_tp > 1 ? n_head    / vm_c_tp : n_head;
    const int64_t n_head_kv_local = vm_c_tp > 1 ? n_head_kv / vm_c_tp : n_head_kv;
#else
    ...
    ggml_tensor * Qcur_full = build_lora_mm(model.layers[il].wq, cur, model.layers[il].wq_s); // [ (n_embd_head * 2) * n_head_local, n_tokens ]
    cb(Qcur_full, "Qcur_full", il);

    ggml_tensor * Qcur = ggml_view_3d(ctx0, Qcur_full, n_embd_head, n_head_local, n_tokens,
        ggml_element_size(Qcur_full) * n_embd_head * 2,
        ggml_element_size(Qcur_full) * n_embd_head * 2 * n_head_local, 0);
    ...
    ggml_tensor * gate = ggml_view_3d(ctx0, Qcur_full, n_embd_head, n_head_local, n_tokens,
        ggml_element_size(Qcur_full) * n_embd_head * 2,
        ggml_element_size(Qcur_full) * n_embd_head * 2 * n_head_local,
        ggml_element_size(Qcur_full) * n_embd_head);
    gate = ggml_cont_2d(ctx0, gate, n_embd_head * n_head_local, n_tokens);
    ...
    // [TP] Wo 是 column-TP 切分，输出需要 allreduce
    add_reduce(cur);
...
ggml_tensor * llama_model_qwen35moe::graph::build_layer_attn_linear(
    ...
    // All local (TP-sharded) dimensions are derived from actual tensor shapes after weight binding
    const int64_t value_dim    = model.layers[il].wqkv_gate->ne[1];
    const int64_t num_v_heads  = model.layers[il].ssm_beta->ne[1];
    const int64_t head_v_dim   = value_dim / num_v_heads;

    const int64_t conv_dim     = model.layers[il].ssm_conv1d->ne[1];
    const int64_t num_k_heads  = (conv_dim - value_dim) / (2 * head_k_dim);

    GGML_ASSERT(n_seqs != 0);
    GGML_ASSERT(ubatch.equal_seqs());
    GGML_ASSERT(ubatch.n_tokens == n_seq_tokens * n_seqs);
    GGML_ASSERT(head_v_dim == head_k_dim && "S_k == S_v required for delta net");
    ...
    const int64_t conv_channels    = conv_kernel->ne[1];  // local (TP-sharded)

    ggml_tensor * conv_input = build_conv_state(inp, conv_states_all, qkv_mixed, conv_kernel_size, conv_channels, il);

    // Use actual tensor dimension for state_size (TP-sharded)
    ggml_tensor * state = build_rs(inp, ssm_states_all, ssm_states_all->ne[0], n_seqs);
    ...
    // [TP] ssm_out 是 column-TP 切分，输出需要 allreduce
    add_reduce(cur);
...
ggml_tensor * llama_model_qwen35moe::graph::build_layer_ffn(ggml_tensor * cur, const int il) {
    ...
    // [TP] FFN down 是 column-TP 切分，输出需要 allreduce
    add_reduce(cur);
...
llama_model_qwen35moe::graph_mtp::graph_mtp(const llama_model & model, const llm_graph_params & params)
    ...
    vm_c::hidden_log_register("mtp.inp_tokens", inp->tokens);

    inp->embd = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, hparams.n_embd_inp(), n_tokens);
    ggml_set_input(inp->embd);
    vm_c::hidden_log_register("mtp.inp_embd", inp->embd);

    std::array<ggml_tensor *, 2> tok_embd_paths;
    {
        ggml_tensor * tok_embd_w = layer.nextn.embed_tokens ? layer.nextn.embed_tokens : model.tok_embd;
        tok_embd_paths[0] = ggml_get_rows(ctx0, tok_embd_w, inp->tokens);
    }
    tok_embd_paths[1] = inp->embd;

    GGML_ASSERT(ggml_are_same_shape(tok_embd_paths[0], tok_embd_paths[1]));
    ggml_tensor * tok_embd = ggml_build_forward_select(gf, tok_embd_paths.data(), tok_embd_paths.size(), ubatch.token ? 0 : 1);
    cb(tok_embd, "mtp_tok_embd", il);
    vm_c::hidden_log_register("mtp.tok_embd", tok_embd);
    ...
    vm_c::hidden_log_register("mtp.inp_h", inp->h);
    ...
    vm_c::hidden_log_register("mtp.h_norm", h_norm);

    ggml_tensor * e_norm = build_norm(tok_embd, layer.nextn.enorm, nullptr, LLM_NORM_RMS, il);
    cb(e_norm, "mtp_enorm", il);
    vm_c::hidden_log_register("mtp.e_norm", e_norm);

    ggml_tensor * concat = ggml_concat(ctx0, e_norm, h_norm, /*dim=*/ 0);
    cb(concat, "mtp_concat", il);
    vm_c::hidden_log_register("mtp.concat", concat);

    ggml_tensor * cur = build_lora_mm(layer.nextn.eh_proj, concat, layer.nextn.eh_proj_s);
    cb(cur, "mtp_eh_proj", il);
    vm_c::hidden_log_register("mtp.eh_proj", cur);

    ggml_tensor * inpSA = cur;

#ifdef VM_C_LLAMA_INTEGRATION
    // TP-sharded head counts (same as build_qkv pattern)
    const int vm_c_tp = (cparams.vm_c_tp && cparams.vm_c_tp_size > 1) ? cparams.vm_c_tp_size : 1;
    const int64_t n_head_local    = vm_c_tp > 1 ? n_head    / vm_c_tp : n_head;
    const int64_t n_head_kv_local = vm_c_tp > 1 ? n_head_kv / vm_c_tp : n_head_kv;
#else
    ...
    ggml_tensor * Qcur = ggml_view_3d(ctx0, Qcur_full,
            n_embd_head, n_head_local, n_tokens,
            ggml_element_size(Qcur_full) * n_embd_head * 2,
            ggml_element_size(Qcur_full) * n_embd_head * 2 * n_head_local,
            0);
    ...
    // [TP] MTP Wo 是 column-TP 切分，输出需要 allreduce
    add_reduce(cur);
    cb(cur, "mtp_attn_out", il);
    vm_c::hidden_log_register("mtp.attn_post_wo", cur);
    ...
    vm_c::hidden_log_register("mtp.attn_post_norm", cur);
    ...
    // [TP] MTP FFN down 是 column-TP 切分，输出需要 allreduce
    add_reduce(cur);
    cb(cur, "mtp_ffn_out", il);

    cur = ggml_add(ctx0, cur, ffn_residual);
    cb(cur, "mtp_post_ffn", il);
    vm_c::hidden_log_register("mtp.post_ffn", cur);
    ...
    vm_c::hidden_log_register("mtp.head_norm", cur);

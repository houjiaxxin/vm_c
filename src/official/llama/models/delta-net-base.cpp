...
ggml_tensor * llm_build_delta_net_base::build_conv_state(
    ...
    // state_size from actual tensor dimension (handles TP sharding)
    ggml_tensor * conv_states = build_rs(inp, conv_states_all, conv_states_all->ne[0], n_seqs);
...
ggml_tensor * llm_build_delta_net_base::build_recurrent_attn(
    ...
    if (!keep) {
        ...
        ggml_build_forward_expand(gf,
                ggml_cpy(ctx0, new_state,
                    ggml_view_2d(ctx0, ssm_states_all, ssm_states_all->ne[0], n_seqs, ssm_states_all->nb[1],
                        kv_head * ssm_states_all->ne[0] * ggml_element_size(ssm_states_all))));
    ...
    const size_t row_size = ssm_states_all->ne[0] * ggml_element_size(ssm_states_all);
    for (int64_t k_i = 0; k_i < K; ++k_i) {
        ...
        ggml_tensor * dst = ggml_view_2d(ctx0, ssm_states_all,
            ssm_states_all->ne[0], n_seqs, ssm_states_all->nb[1],

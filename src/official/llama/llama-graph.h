...
struct llm_graph_params {
    ...
    void  * tp_ctx  = nullptr; // NcclComm 实例指针
    int32_t tp_size = 1;       // world size
...
struct llm_graph_context {
    ...
    //   tp_ctx_  : NcclComm 实例指针（void* 以避免依赖 distributed 头文件）
    //   tp_size_ : TP world size（1 = 单卡，不执行 allreduce）
    void  * tp_ctx_  = nullptr;
    int32_t tp_size_ = 1;

    // 在 Wo / FFN down / lm_head 输出后插入 ggml_vm_c_tp_allreduce 节点
    // 当 tp_size_ > 1 时，CUDA backend 通过注册的回调执行真正的 NCCL/internal allreduce
    void add_reduce(ggml_tensor *& t);
    ...
    // 当 KV cache 索引 tensor (k_idxs / v_idxs / kq_mask) 不会被 graph op 引用时
    // (例如被 TurboQuant 等模块接管), 传入 skip_kv_idxs=true 避免创建死代码。
    llm_graph_input_attn_kv * build_attn_inp_kv(bool skip_kv_idxs) const;

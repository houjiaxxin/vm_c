...
static llama_model * llama_model_mapping(llm_arch arch, const llama_model_params & params) {
    // vm_c: 仅支持 LLAMA 和 QWEN35MOE 架构。
    // 官方 llama.cpp 源码在 D:\vm_c\llama.cpp\src\models\，
    // 如需添加新架构，将对应 .cpp 拷贝到 src/official/llama/models/ 并在此添加 case。
    switch (arch) {
        case LLM_ARCH_LLAMA:
        ...
        default:
            throw std::runtime_error(
                std::string("vm_c: unsupported model architecture '") + llm_arch_name(arch)
                + "' — add model file from llama.cpp/src/models/ and add case here");
    }
}
...
bool llama_model_base::load_tensors(llama_model_loader & ml) {
    ...
    int layer_assign_gpu = 0;
    int layer_assign_cpu = 0;
    ...
    LLAMA_LOG_INFO("%s: layer device map: %d on GPU, %d on CPU (n_layer=%d, gpu_layers=%d)\n",
        __func__, layer_assign_gpu, layer_assign_cpu, n_layer, n_gpu_layers);
    ...
    if (llama_supports_gpu_offload()) {
        ...
        const bool offload_output = n_repeating > 0;
        if (offload_output) {
            n_repeating--;
        }

        const int max_backend_supported_layers = hparams.n_layer + 1;
        const int offloaded = std::min(n_gpu_layers, max_backend_supported_layers);

        LLAMA_LOG_INFO("%s: GPU offload %d/%d layers (output=%s, repeating=%d)\n", __func__,
            offloaded, max_backend_supported_layers, offload_output ? "yes" : "no", n_repeating);
    }

    // print memory requirements per buffer type (one summary line)
    {
        std::string bufs_summary;
        for (auto & [_, bufs] : pimpl->ctxs_bufs) {
            for (auto & buf: bufs) {
                if (!bufs_summary.empty()) {
                    bufs_summary += ", ";
                }
                bufs_summary += format("%s=%.2f MiB",
                    ggml_backend_buffer_name(buf.get()),
                    ggml_backend_buffer_get_size(buf.get()) / 1024.0 / 1024.0);
            }
        }
        if (!bufs_summary.empty()) {
            LLAMA_LOG_INFO("%s: model buffers: %s\n", __func__, bufs_summary.c_str());
        }
    }
        ...
        default:
            {
                ...
                if (llm_arch_is_recurrent(arch)) {
                    const int32_t vm_c_tp_size =
#ifdef VM_C_LLAMA_INTEGRATION
                        (cparams.vm_c_tp && cparams.vm_c_tp_size > 1) ? cparams.vm_c_tp_size : 1;
#else
                        1;
#endif
                    ...
                    if (arch == LLM_ARCH_FALCON_H1) {
                        ...
                        filter_attn = [&, n_main](int32_t il) {
                            if ((uint32_t) il >= n_main || hparams.is_recurrent(il)) {
                                return false;
                            }
#ifdef VM_C_LLAMA_INTEGRATION
                            if (cparams.vm_c_tq_bridge != nullptr &&
                                vm_c_tq_bridge_layer_uses_turboquant(cparams.vm_c_tq_bridge, il)) {
                                return false;
                            }
#endif
                            return true;
                    ...
                    // [vm_c] per-layer filter 摘要（hybrid 路径），用于定位 TurboQuant 路径
                    {
                        int n_attn = 0, n_recr = 0;
                        for (uint32_t il = 0; il < hparams.n_layer; il++) {
                            if (filter_attn && filter_attn((int32_t)il)) ++n_attn;
                            if (filter_recr && filter_recr((int32_t)il)) ++n_recr;
                        }
                        LLAMA_LOG_INFO("[FILTER-SUMMARY] arch=hybrid n_layer=%u filter_attn_keep=%d filter_recr_keep=%d "
                                       "tq_bridge_set=%d\n",
                                       hparams.n_layer, n_attn, n_recr,
                                       cparams.vm_c_tq_bridge != nullptr ? 1 : 0);
                    }
                } else {
                    ...
                    // [vm_c] per-layer filter 摘要（非-hybrid 路径，例如 MTP context）
                    if (filter) {
                        int n_keep = 0;
                        for (uint32_t il = 0; il < hparams.n_layer; il++) {
                            if (filter((int32_t)il)) ++n_keep;
                        }
                        LLAMA_LOG_INFO("[FILTER-SUMMARY] arch=plain n_layer=%u filter_keep=%d\n",
                                       hparams.n_layer, n_keep);
                    }
...
int32_t llama_model_n_embd_head_k(const llama_model * model) {
    return (int32_t) model->hparams.n_embd_head_k(0);
}

int32_t llama_model_n_embd_head_v(const llama_model * model) {
    return (int32_t) model->hparams.n_embd_head_v(0);
}

...
#include "vm_c/official/vm_c_llama_graph_ext.h"
#endif
...
// ggml_vm_c_tp_allreduce 函数声明（ggml.h 中声明）
// add_reduce 实现 — 当 tp_size_ > 1 时在图中插入 allreduce 节点
void llm_graph_context::add_reduce(ggml_tensor *& t) {
    // [vm_c 诊断] 同一 (name, shape) 只在首次出现时打一行 + 进程级累计计数；
    // 每 128 次额外打一次以观察增长。VMC_LOG_TP=1 开启。
    ...
    if (dbg && t) {
        ...
        if (t->name && t->name[0]) std::snprintf(sname, sizeof(sname), "%s", t->name);
        char shape[80] = "?";
        std::snprintf(shape, sizeof(shape), "[%lld,%lld,%lld,%lld]",
                      (long long) t->ne[0], (long long) t->ne[1],
                      (long long) t->ne[2], (long long) t->ne[3]);
        // 按 (name, shape) 累计；用 static 进程级 map
        static std::unordered_map<std::string, int> cnt;
        const std::string key = std::string(sname) + "@" + shape;
        int c = ++cnt[key];
        if (c == 1 || (c % 128) == 0) {
            fprintf(stderr,
                    "[VMC-TP] add_reduce tp_size_=%d name=%s shape=%s total=%d\n",
                    (int) tp_size_, sname, shape, c);
        }
    }
    if (tp_size_ > 1) {
        t = ggml_vm_c_tp_allreduce(ctx0, t, tp_ctx_, tp_size_);
    }
}
...
void llm_graph_input_embd::set_input(const llama_ubatch * ubatch) {
    if (ubatch->token) {
        ...
        if (tokens) {
            LLAMA_LOG_INFO("[SET-INPUT-embd] n_tokens=%d ubatch.token[0]=%d (set tokens tensor, buf=%p, data=%p, ne[0]=%lld)\n",
                (int)n_tokens, ubatch->token[0], (void*)tokens->buffer, (void*)tokens->data, (long long)tokens->ne[0]);
        }
        ggml_backend_tensor_set(tokens, ubatch->token, 0, n_tokens*ggml_element_size(tokens));
        if (tokens && ubatch->n_tokens > 0) {
            int32_t verify[1] = {-1};
            ggml_backend_tensor_get(tokens, verify, 0, sizeof(int32_t));
            LLAMA_LOG_INFO("[SET-INPUT-embd-VERIFY] tokens[0] after set = %d\n", verify[0]);
        }
    }
    ...
    if (ubatch->embd) {
        ...
        LLAMA_LOG_INFO("[SET-INPUT-embd] n_tokens=%d ubatch.embd[0]=%.4f (set embd tensor, ne[0]=%d)\n", (int)n_tokens, ubatch->embd[0], (int)embd->ne[0]);
...
void llm_graph_input_embd_h::set_input(const llama_ubatch * ubatch) {
    ...
    if (ubatch->token) {
        if (tokens) {
            LLAMA_LOG_INFO("[SET-INPUT-embd_h] n_tokens=%d ubatch.token[0]=%d ubatch.embd_ptr=%p (set tokens tensor, buf=%p, data=%p, ne[0]=%lld, nb[0]=%zu)\n",
                (int)n_tokens, ubatch->token[0], (void*)ubatch->embd, (void*)tokens->buffer, (void*)tokens->data,
                (long long)tokens->ne[0], tokens->nb[0]);
        }
        ggml_backend_tensor_set(tokens, ubatch->token, 0, n_tokens*ggml_element_size(tokens));
        if (tokens) {
            int32_t verify[1] = {-1};
            ggml_backend_tensor_get(tokens, verify, 0, sizeof(int32_t));
            LLAMA_LOG_INFO("[SET-INPUT-embd_h-VERIFY] tokens[0] after set = %d\n", verify[0]);
        }
    } else {
        GGML_ASSERT(ubatch->embd);
        GGML_ASSERT(n_embd == embd->ne[0]);

        LLAMA_LOG_INFO("[SET-INPUT-embd_h] n_tokens=%d ubatch.token=NULL ubatch.embd[0]=%.4f (set embd tensor)\n", (int)n_tokens, ubatch->embd[0]);
...
void llm_graph_input_attn_kv::set_input(const llama_ubatch * ubatch) {
    // tensor 可能因 turboquant 等路径提前 return 而未分配 buffer，
    // 此时不能对其写数据（参见 ISWA 路径的相同模式）
...
void llm_graph_input_mem_hybrid::set_input(const llama_ubatch * ubatch) {
    // 交由 llm_graph_input_attn_kv::set_input 处理 — 当 KV cache 无活跃层时
    // 该函数内部通过 null tensor 检查跳过未创建的那些输入（根因修复）
...
bool llm_graph_input_mem_hybrid::can_reuse(const llm_graph_params & params) {
    ...
    // 当 KV cache 无活跃层时，inp_attn 中的 tensor 未被创建，跳过检查
...
llm_graph_context::llm_graph_context(const llm_graph_params & params) :
    ...
    tp_ctx_          (params.tp_ctx),
    tp_size_         (params.tp_size),
    cb_func          (params.cb),
    res              (params.res),
    ctx0             (res->get_ctx()),
    gf               (res->get_gf()) {
...
llm_graph_qkv llm_graph_context::build_qkv(
...
#ifdef VM_C_LLAMA_INTEGRATION
    const int vm_c_tp = (cparams.vm_c_tp && cparams.vm_c_tp_size > 1) ? cparams.vm_c_tp_size : 1;
    const int64_t n_head_local    = vm_c_tp_local_dim(n_head,    vm_c_tp);
    const int64_t n_head_kv_local = vm_c_tp_local_dim(n_head_kv, vm_c_tp);
#else
    const int64_t n_head_local    = n_head;
    const int64_t n_head_kv_local = n_head_kv;
#endif
    ...
    if (layer.wqkv) {
        ...
        Qcur = ggml_view_3d(ctx0, qkv, n_embd_head, n_head_local,    n_tokens,
            ggml_row_size(qkv->type, n_embd_head), qkv->nb[1], 0);
        Kcur = ggml_view_3d(ctx0, qkv, n_embd_head, n_head_kv_local, n_tokens,
            ggml_row_size(qkv->type, n_embd_head), qkv->nb[1],
            ggml_row_size(qkv->type, n_embd_head * n_head_local));
        Vcur = ggml_view_3d(ctx0, qkv, n_embd_head, n_head_kv_local, n_tokens,
            ggml_row_size(qkv->type, n_embd_head), qkv->nb[1],
            ggml_row_size(qkv->type, n_embd_head * n_head_local + n_embd_head * n_head_kv_local));
    } else {
        // separate Q/K/V path
...
static std::unique_ptr<llm_graph_input_attn_kv> build_attn_inp_kv_impl(
    ...
    bool skip_kv_idxs = false) {

    auto inp = std::make_unique<llm_graph_input_attn_kv>(hparams, cparams, mctx_cur);

    // 当 KV cache 中没有活跃层时（例如所有 full-attention 层被 TurboQuant 过滤），
    // 不创建 k_idxs / v_idxs / kq_mask 等输入 tensor。
    // 这些 tensor 不会被任何 graph op 引用，allocator 会跳过它们导致 NULL buffer。
    // 这是根因修复，而非打补丁。
    // skip_kv_idxs 由特殊 graph 类型（如 MTP）在确定 KV cache 由其它模块
    //（如 TurboQuant）接管时传入 true。
    if (mctx_cur->get_n_layers() > 0 && !skip_kv_idxs) {
...
llm_graph_input_attn_kv * llm_graph_context::build_attn_inp_kv(bool skip_kv_idxs) const {
    const auto * mctx_cur = static_cast<const llama_kv_cache_context *>(mctx);

    auto inp = build_attn_inp_kv_impl(ctx0, ubatch, hparams, cparams, mctx_cur, skip_kv_idxs);
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    {
#ifdef VM_C_LLAMA_INTEGRATION
        if (cparams.vm_c_kv_quant_method != nullptr &&
            std::strstr(cparams.vm_c_kv_quant_method, "turboquant") != nullptr &&
            cparams.vm_c_tq_bridge != nullptr) {
            ggml_tensor * kv = vm_c_tq_bridge_kv_tensor(cparams.vm_c_tq_bridge, il);
            if (kv) {
                ggml_tensor * cur = ggml_vm_c_turboquant_attn(
                    ctx0, q_cur, k_cur, v_cur, kv, il, kq_scale, cparams.vm_c_tq_bridge);
                cb(cur, "kqv_out", il);
                ...
                // [vm_c] 累计 TurboQuant 路径命中次数（每个 layer 第一次构建时打印）
                static std::set<int> _vm_c_tq_layers_logged;
                if (_vm_c_tq_layers_logged.insert(il).second) {
                    LLAMA_LOG_INFO("[ATTN-PATH] il=%d path=TurboQuant kv_tensor=%p slot_size_inferred=%d\n",
                                   il, (void*)kv,
                                   /* 提醒：与 kv_cache_mgr 的 slot_size_aligned 必须一致 */
                                   0);
                }
                return cur;
            } else {
                // [vm_c] TQ bridge 在但该层无 kv tensor（说明 is_layer_turboquant==false 或
                // 桥接尚未 init）—— 强制走标准 MHA 路径；若标准 KV cache 也无此层则报错
                static std::set<int> _vm_c_fallback_logged;
                if (_vm_c_fallback_logged.insert(il).second) {
                    LLAMA_LOG_WARN("[ATTN-PATH] il=%d path=Standard-FALLBACK TQ-bridge-set-but-no-kv-tensor\n", il);
                }
            }
        }
#endif
...
void llm_graph_context::build_sampling() const {
    // no backend samplers configured
}

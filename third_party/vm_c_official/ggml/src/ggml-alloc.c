...
// [vm_c] 内部 [DEBUG] 日志开关。默认关，需要时设 VMC_LOG_GALLOC=1。
//   - VMC_LOG_GALLOC=1  : 打印所有 [DEBUG] 行（needs_realloc / gallocr_alloc_graph）
//   - 未设或 0          : 完全静默
static int vmc_log_galloc_enabled(void) {
    static int cached = -1;
    if (cached < 0) {
        const char * env = getenv("VMC_LOG_GALLOC");
...
static bool ggml_gallocr_reserve_n_impl(
    ...
    for (int i = 0; i < graph->n_nodes; i++) {
        ...
        if (node->view_src || node->data || node->buffer) {
        ...
        for (int j = 0; j < GGML_MAX_SRC; j++) {
            struct ggml_tensor * src = node->src[j];
            if (!src || src->view_src || src->data || src->buffer) {
    ...
    for (int i = 0; i < graph->n_leafs; i++) {
        ...
        if (leaf->view_src || leaf->data || leaf->buffer) {
...
static void ggml_gallocr_init_tensor(ggml_gallocr_t galloc, struct ggml_tensor * tensor, struct tensor_alloc * tensor_alloc) {
    ...
    if (buffer_id < 0) {
        // 即使是 buffer_id < 0 的 tensor（被标注为"已有外部 buffer"），
        // 其 view 的 buffer 字段可能仍为 NULL（ggml_new_tensor_impl 初始化为 NULL），
        // 需要调用 ggml_backend_view_init 从 view_src 继承 buffer 指针。
        // 不初始化的话，ggml_cuda_graph_evaluate_and_capture 中 assert 会因
        // node->src[j]->buffer == NULL 而崩溃。
        if (tensor->view_src != NULL && tensor->buffer == NULL && tensor->view_src->buffer != NULL) {
            ggml_backend_view_init(tensor);
        }
        return;
    }
    // no_alloc dummy / external WEIGHTS buffer：已有 buffer 占位，不可再进 compute pool
...
static bool ggml_gallocr_needs_realloc(ggml_gallocr_t galloc, struct ggml_cgraph * graph) {
    if (galloc->n_nodes != graph->n_nodes) {
        if (vmc_log_galloc_enabled()) {
            fprintf(stderr, "[DEBUG] needs_realloc: n_nodes mismatch galloc=%d graph=%d\n", galloc->n_nodes, graph->n_nodes);
        }
        return true;
    }
    ...
    if (galloc->n_leafs != graph->n_leafs) {
        if (vmc_log_galloc_enabled()) {
            fprintf(stderr, "[DEBUG] needs_realloc: n_leafs mismatch galloc=%d graph=%d\n", galloc->n_leafs, graph->n_leafs);
        }
        return true;
    }
    ...
    for (int i = 0; i < graph->n_nodes; i++) {
        ...
        if (!ggml_gallocr_node_needs_realloc(galloc, node, &node_alloc->dst)) {
            size_t dst_size = 0;
            if (!node->data && !node->view_src && node_alloc->dst.buffer_id >= 0) {
                dst_size = ggml_backend_buft_get_alloc_size(galloc->bufts[node_alloc->dst.buffer_id], node);
            }
            if (vmc_log_galloc_enabled()) {
                fprintf(stderr, "[DEBUG] needs_realloc: node[%d] dst '%s' op=%s size_max=%zu node_size=%zu buffer_id=%d data=%p view_src=%p\n",
                        i, node->name, ggml_op_name(node->op), node_alloc->dst.size_max, dst_size,
                        node_alloc->dst.buffer_id, (void *)node->data, (void *)node->view_src);
            }
            return true;
        }
        ...
        for (int j = 0; j < GGML_MAX_SRC; j++) {
            ...
            if (!ggml_gallocr_node_needs_realloc(galloc, src, &node_alloc->src[j])) {
                size_t src_size = 0;
                if (!src->data && !src->view_src && node_alloc->src[j].buffer_id >= 0) {
                    src_size = ggml_backend_buft_get_alloc_size(galloc->bufts[node_alloc->src[j].buffer_id], src);
                }
                if (vmc_log_galloc_enabled()) {
                    fprintf(stderr, "[DEBUG] needs_realloc: node[%d] '%s' op=%s src[%d] '%s' size_max=%zu node_size=%zu buffer_id=%d data=%p view_src=%p buffer=%p\n",
                            i, node->name, ggml_op_name(node->op), j, src->name, node_alloc->src[j].size_max, src_size,
                            node_alloc->src[j].buffer_id, (void *)src->data, (void *)src->view_src, (void *)src->buffer);
                }
                return true;
            }
            if (vmc_log_galloc_enabled() && src->name && strncmp(src->name, "cache_", 6) == 0) {
                fprintf(stderr, "[DEBUG] needs_realloc: OK node[%d] '%s' op=%s src[%d] '%s' buffer_id=%d data=%p view_src=%p buffer=%p\n",
                        i, node->name, ggml_op_name(node->op), j, src->name, node_alloc->src[j].buffer_id,
                        (void *)src->data, (void *)src->view_src, (void *)src->buffer);
            }
        }
    }
...
bool ggml_gallocr_alloc_graph(ggml_gallocr_t galloc, struct ggml_cgraph * graph) {
    bool needs_realloc = ggml_gallocr_needs_realloc(galloc, graph);
    if (vmc_log_galloc_enabled()) {
        fprintf(stderr, "[DEBUG] gallocr_alloc_graph: needs_realloc=%d n_buffers=%d galloc->n_nodes=%d graph->n_nodes=%d galloc->n_leafs=%d graph->n_leafs=%d\n",
                needs_realloc, galloc->n_buffers, galloc->n_nodes, graph->n_nodes, galloc->n_leafs, graph->n_leafs);
    }
    if (needs_realloc) {

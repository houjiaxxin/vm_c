...
void * ggml_backend_event_get_context(ggml_backend_event_t event) {
    GGML_ASSERT(event);
...
static bool ggml_backend_sched_alloc_splits(ggml_backend_sched_t sched) {
    ...
    // vm_c 优化：alloc_splits 每步打印严重刷屏，改为环境变量控制
    //   VMC_LOG_ALLOC_SPLITS=1            打印每步（默认 0）
    //   VMC_LOG_ALLOC_SPLITS_EVERY=N      每 N 步打印一次
    //   VMC_LOG_ALLOC_SPLITS_REALLOC=1    仅 backend_ids_changed 时打印（即使日志关闭也强制打印 alloc 失败）
    static const char* env_alloc_always  = getenv("VMC_LOG_ALLOC_SPLITS");
    static const char* env_alloc_every   = getenv("VMC_LOG_ALLOC_SPLITS_EVERY");
    static const char* env_alloc_realloc = getenv("VMC_LOG_ALLOC_SPLITS_REALLOC");
    static thread_local int alloc_step_counter = 0;
    ++alloc_step_counter;
    bool alloc_log_now = false;
    if (env_alloc_always && env_alloc_always[0] == '1') {
        alloc_log_now = true;
    } else if (env_alloc_every && atoi(env_alloc_every) > 0
               && (alloc_step_counter % atoi(env_alloc_every)) == 0) {
        alloc_log_now = true;
    } else if (env_alloc_realloc && env_alloc_realloc[0] == '1' && backend_ids_changed) {
        alloc_log_now = true;
    }
    if (alloc_log_now) {
        fprintf(stderr, "[DEBUG] alloc_splits: step=%d backend_ids_changed=%d n_nodes=%d n_leafs=%d\n",
                alloc_step_counter, backend_ids_changed, sched->graph.n_nodes, sched->graph.n_leafs);
    }

    bool alloc_ok = ggml_gallocr_alloc_graph(sched->galloc, &sched->graph);
    if (alloc_log_now || (!alloc_ok && env_alloc_realloc && env_alloc_realloc[0] == '1')) {
        fprintf(stderr, "[DEBUG] alloc_splits: step=%d first ggml_gallocr_alloc_graph = %d\n",
                alloc_step_counter, alloc_ok);
    }

    if (backend_ids_changed || !alloc_ok) {
        ...
        if (alloc_log_now) {
            fprintf(stderr, "[DEBUG] alloc_splits: after ggml_gallocr_reserve_n, retrying alloc_graph\n");
        }
        if (!ggml_gallocr_alloc_graph(sched->galloc, &sched->graph)) {
            if (alloc_log_now) {
                fprintf(stderr, "[DEBUG] alloc_splits: second ggml_gallocr_alloc_graph FAILED\n");
            }
            return false;
        }
    }
...
static enum ggml_status ggml_backend_sched_compute_splits(ggml_backend_sched_t sched) {
    ...
    for (int split_id = 0; split_id < sched->n_splits; split_id++) {
        ...
        for (int input_id = 0; input_id < split->n_inputs; input_id++) {
            ...
            // Skip tensors not allocated by the scheduler (e.g. unused KV inputs in TurboQuant path)
            if (!input->buffer) {
                continue;
            }

...
ggml_cuda_ar_pipeline * ggml_cuda_ar_pipeline_init(const int * devices, size_t n_devices) {
    ...
    for (size_t i = 0; i < n_devices; ++i) {
        if (!ggml_cuda_info().devices[devices[i]].supports_internal_allreduce) {
            GGML_LOG_DEBUG("%s: internal AllReduce requires compute capability >= %d "
                           "(device %d has cc=%d); falling back\n",
                           __func__, GGML_CUDA_CC_VOLTA, devices[i],
                           ggml_cuda_info().devices[devices[i]].cc);
            return nullptr;
        }
    }
...
bool ggml_cuda_ar_allreduce(
    ...
    if (use_copy_engine) {
        ...
        for (int64_t chunk_start = 0; chunk_start < ne; chunk_start += (int64_t) max_chunk_elems) {
            ...
            for (int i = 0; i < n; ++i) {
                const int dev_id = p->devices[i];
                const int peer   = 1 - i;  // valid for n == 2 only

                // Serialize CUDA driver access per-device: the other rank's
                // thread may be on this device doing operations (e.g.
                // cudaFuncSetAttribute) that can deadlock with our
                // cuLaunchKernel in the CUDA driver internals.  Lock is
                // released after all operations on this device are enqueued.
                {
                    std::lock_guard<std::mutex> lock(g_cuda_dev_mutex[dev_id]);

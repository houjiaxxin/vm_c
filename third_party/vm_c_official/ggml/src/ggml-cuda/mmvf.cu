...
void ggml_cuda_mul_mat_vec_f(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * ids, ggml_tensor * dst,
    ...
    const enum ggml_prec prec = device_has_fast_fp16(ggml_cuda_get_device()) ? ggml_prec(dst->op_params[0]) : GGML_PREC_F32;
...
void ggml_cuda_op_mul_mat_vec_f(
    ...
    const enum ggml_prec prec = device_has_fast_fp16(id) ? ggml_prec(dst->op_params[0]) : GGML_PREC_F32;
...
bool ggml_cuda_should_use_mmvf(enum ggml_type type, int device_id, const int64_t * src0_ne, const size_t * src0_nb, int64_t ne11) {
    ...
    switch (type) {
        ...
        case GGML_TYPE_F16:
            if (GGML_CUDA_CC_IS_NVIDIA(cc)) {
                ...
                if (device_has_fp16_mma(device_id)) {
        ...
        case GGML_TYPE_BF16:
            if (GGML_CUDA_CC_IS_NVIDIA(cc)) {
                ...
                if (device_has_bf16_mma(device_id)) {
                    return src0_small && ne11 <= 3;
                }
                return ne11 <= 8;
            } else if (GGML_CUDA_CC_IS_AMD(cc)) {

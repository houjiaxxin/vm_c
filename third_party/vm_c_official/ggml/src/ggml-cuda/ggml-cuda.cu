...
// Per-device mutex for serializing CUDA driver operations in multi-threaded TP.
// This is the VMC_TODO item: "vm_c's multi-threaded TP calls into the same
// CUDA context from two threads; protect against driver-internal deadlocks".
std::mutex g_cuda_dev_mutex[GGML_CUDA_MAX_DEVICES];

// Global process-level mutex for CUDA driver operations that have a process-wide
// internal lock (cuKernelGetFunction).  Per-device mutexes cannot prevent deadlock
// when two threads on different GPUs enter the driver simultaneously.
std::mutex g_cuda_global_mutex;
...
static ggml_cuda_device_info ggml_cuda_init() {
    ...
    // ========== 硬件环境适配模块：启动时环境检测 ==========
    GGML_LOG_INFO("%s: ===== Hardware Environment & Adaptation =====\n", __func__);
    GGML_LOG_INFO("%s:   CUDA Runtime version : %d (compile-time CUDART_VERSION)\n", __func__, info.cudart_version);
#if defined(GGML_USE_NCCL)
    GGML_LOG_INFO("%s:   NCCL enabled         : yes\n", __func__);
#else
    GGML_LOG_INFO("%s:   NCCL enabled         : no\n", __func__);
#endif
#if defined(GGML_USE_VMM)
    GGML_LOG_INFO("%s:   VMM enabled          : yes\n", __func__);
#else
    GGML_LOG_INFO("%s:   VMM enabled          : no\n", __func__);
#endif
    GGML_LOG_INFO("%s: ============================================\n", __func__);
    ...
    for (int id = 0; id < info.device_count; ++id) {
        ...
        // ========== 启动时根据计算能力推导所有设备适配策略 ==========
        const int dev_cc = info.devices[id].cc;

        // 策略 1: TF32 张量核心 + cuBLAS 数学模式
        // TF32 需要 Ampere (sm_80+) 架构；sm_75 使用 TF32 会导致 cublasSgemm 死锁
        if (GGML_CUDA_CC_IS_NVIDIA(dev_cc) && dev_cc >= GGML_CUDA_CC_AMPERE) {
            info.devices[id].has_tf32_tensor_core = true;
            info.devices[id].cublas_math_mode = CUBLAS_TF32_TENSOR_OP_MATH;
        } else {
            info.devices[id].has_tf32_tensor_core = false;
            info.devices[id].cublas_math_mode = CUBLAS_DEFAULT_MATH;
        }

        // 策略 2: CUDA Graph 支持（Ampere+ 才安全）
        info.devices[id].supports_cuda_graph =
            GGML_CUDA_CC_IS_NVIDIA(dev_cc) && dev_cc >= GGML_CUDA_CC_AMPERE;

        // 策略 3: 内部 AllReduce 内核（Volta+ 才可用，依赖 __nanosleep）
        info.devices[id].supports_internal_allreduce =
            GGML_CUDA_CC_IS_NVIDIA(dev_cc) && dev_cc >= GGML_CUDA_CC_VOLTA;

        // ========== 编译时 GPU 能力预计算（基于 ggml_cuda_highest_compiled_arch） ==========
        info.devices[id].highest_compiled_arch = ggml_cuda_highest_compiled_arch(dev_cc);
        const int hca = info.devices[id].highest_compiled_arch;

        info.devices[id].has_fp16        = fp16_available(dev_cc);
        info.devices[id].has_fast_fp16   = fast_fp16_available(dev_cc);
        info.devices[id].has_fast_fp16_hw = fast_fp16_hardware_available(dev_cc);
        info.devices[id].has_fp16_mma    = fp16_mma_hardware_available(dev_cc);
        info.devices[id].has_bf16_mma    = bf16_mma_hardware_available(dev_cc);
        info.devices[id].has_volta_mma   = volta_mma_available(dev_cc);
        info.devices[id].has_turing_mma  = turing_mma_available(dev_cc);
        info.devices[id].has_ampere_mma  = ampere_mma_available(dev_cc);
        info.devices[id].has_blackwell_mma = blackwell_mma_available(dev_cc);
        info.devices[id].has_cp_async    = cp_async_available(dev_cc);

        // 输出每设备适配摘要
        GGML_LOG_INFO("  Device %d: adapt cublas_mode=%s cuda_graph=%d allreduce=%d"
                      " fp16=%d fast_fp16=%d fp16_mma=%d bf16_mma=%d"
                      " turing_mma=%d ampere_mma=%d\n",
            id,
            info.devices[id].has_tf32_tensor_core ? "TF32" : "FP32",
            info.devices[id].supports_cuda_graph,
            info.devices[id].supports_internal_allreduce,
            info.devices[id].has_fp16,
            info.devices[id].has_fast_fp16,
            info.devices[id].has_fp16_mma,
            info.devices[id].has_bf16_mma,
            info.devices[id].has_turing_mma,
            info.devices[id].has_ampere_mma);
    }
...
static void ggml_cuda_op_mul_mat_cublas(
    ...
    } else if (device_has_fast_fp16_hw(id) && use_fp16) {
...
static bool ggml_cuda_should_fuse_mul_mat_vec_f(const ggml_tensor * tensor) {
    ...
    use_mul_mat_vec_f = use_mul_mat_vec_f && ggml_cuda_should_use_mmvf(src0->type, ggml_cuda_get_device(), src0->ne, src0->nb, is_mul_mat_id ? src1->ne[2] : src1->ne[1]);
...
static void ggml_cuda_mul_mat(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    if (split) {
        ...
        for (int id = 0; id < ggml_backend_cuda_get_device_count(); ++id) {
            ...
            use_mul_mat_q           = use_mul_mat_q             && ggml_cuda_should_use_mmq(src0->type, id, src1->ne[1], /*n_experts=*/0);
            use_mul_mat_f           = use_mul_mat_f             && ggml_cuda_should_use_mmf(src0->type, id, warp_size, src0->ne, src0->nb, src1->ne[1], /*mul_mat_id=*/false);
            use_mul_mat_vec_f       = use_mul_mat_vec_f         && ggml_cuda_should_use_mmvf(src0->type, id, src0->ne, src0->nb, src1->ne[1]);
            any_gpus_with_slow_fp16 = any_gpus_with_slow_fp16   || !device_has_fast_fp16_hw(id);
        }
    } else {
        const int warp_size     = ggml_cuda_info().devices[ctx.device].warp_size;
        use_mul_mat_q           = use_mul_mat_q             && ggml_cuda_should_use_mmq(src0->type, ctx.device, src1->ne[1], /*n_experts=*/0);
        use_mul_mat_f           = use_mul_mat_f             && ggml_cuda_should_use_mmf(src0->type, ctx.device, warp_size, src0->ne, src0->nb, src1->ne[1], /*mul_mat_id=*/false);
        use_mul_mat_vec_f       = use_mul_mat_vec_f         && ggml_cuda_should_use_mmvf(src0->type, ctx.device, src0->ne, src0->nb, src1->ne[1]);
        any_gpus_with_slow_fp16 = any_gpus_with_slow_fp16   || !device_has_fast_fp16_hw(ctx.device);
    }
    ...
    bool use_batched_cublas_bf16 = src0->type == GGML_TYPE_BF16 && device_has_bf16_mma(ggml_cuda_get_device());
    bool use_batched_cublas_f32  = src0->type == GGML_TYPE_F32;

    // [DIAG] dispatch decision log (limit 50 to avoid flooding)
...
static void ggml_cuda_mul_mat_id(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32) {
        ...
        if (ggml_cuda_should_use_mmq(src0->type, id, ne12, /*n_experts=*/ne02)) {
            ggml_cuda_mul_mat_q(ctx, src0, src1, ids, dst);
            return;
        }

        if (ggml_cuda_should_use_mmf(src0->type, id, WARP_SIZE, src0->ne, src0->nb, src1->ne[2], /*mul_mat_id=*/true)) {
    ...
    const ggml_type type_src1_sorted = (src0->type == GGML_TYPE_F16 && !device_has_fast_fp16_hw(ggml_cuda_get_device()))
    ...
    while (!graph_evaluated_or_captured) {
        ...
        if (!use_cuda_graph || cuda_graph_update_required) {
            ...
            for (int i = 0; i < cgraph->n_nodes; i++) {
                ...
                // 跳过 output 为 NULL buffer 的 node（极少数由 gallocr 未分配的临时 tensor）。
                // 根因修复：ggml_gallocr_init_tensor 现在正确初始化了所有 view tensor 的 buffer。
                if (!node->buffer) {
                    GGML_LOG_WARN("%s: skipping node '%s' (op=%s) with NULL buffer\n",
                        __func__, node->name ? node->name : "?", ggml_op_name(node->op));
                    continue;
                }
...
static bool ggml_cuda_graph_set_enabled(ggml_backend_cuda_context * cuda_ctx, const void * graph_key) {
    ...
    if (graph->graph == nullptr) {
        if (!ggml_cuda_info().devices[cuda_ctx->device].supports_cuda_graph) {

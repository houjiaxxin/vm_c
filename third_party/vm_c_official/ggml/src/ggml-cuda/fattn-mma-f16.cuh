...
static __host__ fattn_mma_config ggml_cuda_fattn_mma_get_config(const int DKQ, const int DV, const int ncols, const int device_id) {
    const int cc = ggml_cuda_info().devices[device_id].cc;
    if (device_has_ampere_mma(device_id)) {
        return ggml_cuda_fattn_mma_get_config_ampere(DKQ, DV, ncols);
    }
    if (device_has_turing_mma(device_id)) {
    ...
    GGML_ASSERT(device_has_volta_mma(device_id));
...
static __host__ int ggml_cuda_fattn_mma_get_nthreads(const int DKQ, const int DV, const int ncols, const int device_id) {
    return ggml_cuda_fattn_mma_get_config(DKQ, DV, ncols, device_id).nthreads;
}
...
static __host__ int ggml_cuda_fattn_mma_get_occupancy(const int DKQ, const int DV, const int ncols, const int device_id) {
    return ggml_cuda_fattn_mma_get_config(DKQ, DV, ncols, device_id).occupancy;
}
...
static __host__ int ggml_cuda_fattn_mma_get_nbatch_fa(const int DKQ, const int DV, const int ncols, const int device_id) {
    return ggml_cuda_fattn_mma_get_config(DKQ, DV, ncols, device_id).nbatch_fa;
}
...
static __host__ int ggml_cuda_fattn_mma_get_nbatch_K2(const int DKQ, const int DV, const int ncols, const int device_id) {
    return ggml_cuda_fattn_mma_get_config(DKQ, DV, ncols, device_id).nbatch_K2;
}
...
static __host__ int ggml_cuda_fattn_mma_get_nbatch_V2(const int DKQ, const int DV, const int ncols, const int device_id) {
    return ggml_cuda_fattn_mma_get_config(DKQ, DV, ncols, device_id).nbatch_V2;
}
...
static __host__ int ggml_cuda_fattn_mma_get_nbatch_combine(const int DKQ, const int DV, const int ncols, const int device_id) {
    return ggml_cuda_fattn_mma_get_config(DKQ, DV, ncols, device_id).nbatch_combine;
}
...
static __host__ int ggml_cuda_fattn_mma_get_nstages_target(const int DKQ, const int DV, const int ncols, const int device_id) {
    return ggml_cuda_fattn_mma_get_config(DKQ, DV, ncols, device_id).nstages_target;
}
...
static __host__ bool ggml_cuda_fattn_mma_get_Q_in_reg(const int DKQ, const int DV, const int ncols, const int device_id) {
    return ggml_cuda_fattn_mma_get_config(DKQ, DV, ncols, device_id).Q_in_reg;
}
...
static __host__ int get_cols_per_warp(const int device_id) {
    const int cc = ggml_cuda_info().devices[device_id].cc;
    if (device_has_turing_mma(device_id) || amd_wmma_available(cc) || amd_mfma_available(cc)) {
        return 16;
    } else {
        // Volta
        return 32;
...
static __host__ int ggml_cuda_fattn_mma_get_nstages(const int DKQ, const int DV, const int ncols1, const int ncols2, const int device_id) {
    return device_has_cp_async(device_id) && ncols2 >= 2 ? ggml_cuda_fattn_mma_get_nstages_target(DKQ, DV, ncols1*ncols2, device_id) : 0;
}
...
void ggml_cuda_flash_attn_ext_mma_f16_case(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const int  nthreads       = ggml_cuda_fattn_mma_get_nthreads      (DKQ, DV, ncols, id);
    const int  nbatch_fa      = ggml_cuda_fattn_mma_get_nbatch_fa     (DKQ, DV, ncols, id);
    const int  nbatch_K2      = ggml_cuda_fattn_mma_get_nbatch_K2     (DKQ, DV, ncols, id);
    const int  nbatch_V2      = ggml_cuda_fattn_mma_get_nbatch_V2     (DKQ, DV, ncols, id);
    const int  nbatch_combine = ggml_cuda_fattn_mma_get_nbatch_combine(DKQ, DV, ncols, id);
    const bool Q_in_reg       = ggml_cuda_fattn_mma_get_Q_in_reg      (DKQ, DV, ncols, id);
    const int  nstages        = ggml_cuda_fattn_mma_get_nstages       (DKQ, DV, ncols1, ncols2, id);

    const int cols_per_warp = std::min(ncols, get_cols_per_warp(id));
    ...
    if (logit_softcap == 0.0f) {
        ...
        if (!shared_memory_limit_raised[id]) {
            std::lock_guard<std::mutex> lock(g_cuda_global_mutex);
            if (!shared_memory_limit_raised[id]) {

...
bool ggml_cuda_should_use_mmf(enum ggml_type type, int device_id, int warp_size, const int64_t * src0_ne,
    ...
    switch (type) {
        case GGML_TYPE_F32:
            return device_has_ampere_mma(device_id) || amd_mfma_available(cc);
        case GGML_TYPE_F16:
            return device_has_volta_mma(device_id) || device_has_turing_mma(device_id) || amd_wmma_available(cc) || amd_mfma_available(cc);
        case GGML_TYPE_BF16:
            return device_has_ampere_mma(device_id) || amd_wmma_available(cc) || amd_mfma_available(cc);
        default:
            return false;
    }
}

...
// Per-device mutex to serialize CUDA driver operations when multiple host
// threads share the same CUDA context (vm_c multi-threaded TP).  Prevents
// driver-internal deadlocks that can arise when one thread calls
// cudaFuncSetAttribute (triggers lazy kernel loading) while another thread
// concurrently launches a kernel on the same device via cuLaunchKernel.
extern std::mutex g_cuda_dev_mutex[GGML_CUDA_MAX_DEVICES];

// Global process-level mutex to protect CUDA driver operations that have a
// process-wide lock (e.g. cuKernelGetFunction).  Per-device mutexes cannot
// prevent deadlocks when two threads on different GPUs simultaneously call
// into the CUDA driver's global kernel function cache.  This global mutex
// avoids the CUDA driver internal deadlock (THREAD_LOCAL lock + PROCESS lock).
extern std::mutex g_cuda_global_mutex;
...
// ========== GPU 能力查询函数（查启动时缓存的设备能力信息） ==========
// device_id 版本：使用 ggml_cuda_init() 启动时预计算的结果
// cc 版本（_from_cc）：在启动初始化期间使用（此时缓存尚未就绪）
...
struct ggml_cuda_device_info {
    ...
    // ========== 全局环境信息 ==========
    int cudart_version;  // CUDA Runtime 版本（编译时 CUDART_VERSION）
    ...
    struct cuda_device_info {
        ...
        // --- 启动时推导的适配策略 ---
        bool    has_tf32_tensor_core;           // supports TF32 tensor core math (sm_80+)
#if !defined(GGML_USE_HIP) && !defined(GGML_USE_MUSA)
        cublasMath_t cublas_math_mode;          // recommended cuBLAS math mode for this device (CUDA)
#else
        int         cublas_math_mode;           // recommended cuBLAS math mode for this device (HIP/MUSA)
#endif
        bool    supports_cuda_graph;            // CUDA graph capture is safe (sm_80+)
        bool    supports_internal_allreduce;    // internal allreduce kernel (sm_70+)

        // --- 编译时已确定的 GPU 能力（预计算自 ggml_cuda_highest_compiled_arch） ---
        int     highest_compiled_arch;          // ggml_cuda_highest_compiled_arch(cc)
        bool    has_fp16;                       // fp16_available(cc)
        bool    has_fast_fp16;                  // fast_fp16_available(cc)
        bool    has_fast_fp16_hw;               // fast_fp16_hardware_available(cc)
        bool    has_fp16_mma;                   // fp16_mma_hardware_available(cc)
        bool    has_bf16_mma;                   // bf16_mma_hardware_available(cc)
        bool    has_volta_mma;                  // volta_mma_available(cc)
        bool    has_turing_mma;                 // turing_mma_available(cc)
        bool    has_ampere_mma;                 // ampere_mma_available(cc)
        bool    has_blackwell_mma;              // blackwell_mma_available(cc)
        bool    has_cp_async;                   // cp_async_available(cc)
    };
...
// 设备 ID 版本（基于 ggml_cuda_device_info 缓存）
// 优先使用，避免运行时重新推导
static inline bool device_has_fp16(int device_id) {
    return ggml_cuda_info().devices[device_id].has_fp16;
}
static inline bool device_has_fast_fp16(int device_id) {
    return ggml_cuda_info().devices[device_id].has_fast_fp16;
}
static inline bool device_has_fast_fp16_hw(int device_id) {
    return ggml_cuda_info().devices[device_id].has_fast_fp16_hw;
}
static inline bool device_has_fp16_mma(int device_id) {
    return ggml_cuda_info().devices[device_id].has_fp16_mma;
}
static inline bool device_has_bf16_mma(int device_id) {
    return ggml_cuda_info().devices[device_id].has_bf16_mma;
}
static inline bool device_has_volta_mma(int device_id) {
    return ggml_cuda_info().devices[device_id].has_volta_mma;
}
static inline bool device_has_turing_mma(int device_id) {
    return ggml_cuda_info().devices[device_id].has_turing_mma;
}
static inline bool device_has_ampere_mma(int device_id) {
    return ggml_cuda_info().devices[device_id].has_ampere_mma;
}
static inline bool device_has_blackwell_mma(int device_id) {
    return ggml_cuda_info().devices[device_id].has_blackwell_mma;
}
static inline bool device_has_cp_async(int device_id) {
    return ggml_cuda_info().devices[device_id].has_cp_async;
}
...
struct ggml_backend_cuda_context {
    ...
    cublasHandle_t cublas_handle(int device) {
        if (cublas_handles[device] == nullptr) {
            ...
            // 使用启动时根据计算能力预缓存的 cuBLAS 数学模式
            // (sm_80+ 使用 CUBLAS_TF32_TENSOR_OP_MATH, 否则使用 CUBLAS_DEFAULT_MATH)
            CUBLAS_CHECK(cublasSetMathMode(cublas_handles[device],
                ggml_cuda_info().devices[device].cublas_math_mode));
        }

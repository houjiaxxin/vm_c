...
void ggml_cuda_mul_mat_q(
    ...
    const bool use_native_fp4 = device_has_blackwell_mma(ggml_cuda_get_device()) && (src0->type == GGML_TYPE_MXFP4 || src0->type == GGML_TYPE_NVFP4);
...
bool ggml_cuda_should_use_mmq(enum ggml_type type, int device_id, int64_t ne11, int64_t n_experts) {
    ...
    if (GGML_CUDA_CC_IS_NVIDIA(cc)) {
        return !device_has_fp16_mma(device_id) || ne11 < MMQ_DP4A_MAX_BATCH_SIZE;
    }

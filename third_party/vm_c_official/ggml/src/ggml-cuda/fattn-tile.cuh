...
static __host__ uint32_t ggml_cuda_fattn_tile_get_config(const int DKQ, const int DV, const int ncols, const int device_id) {
    ...
    if (device_has_fast_fp16(device_id)) {
...
static __host__ int ggml_cuda_fattn_tile_get_nthreads(const int DKQ, const int DV, const int ncols, const int device_id) {
    return (ggml_cuda_fattn_tile_get_config(DKQ, DV, ncols, device_id) >> 0) & ((1 << 10) - 1);
}
...
static __host__ int ggml_cuda_fattn_tile_get_occupancy(const int DKQ, const int DV, const int ncols, const int device_id) {
    return (ggml_cuda_fattn_tile_get_config(DKQ, DV, ncols, device_id) >> 10) & ((1 << 4) - 1);
}
...
static __host__ int ggml_cuda_fattn_tile_get_nbatch_fa(const int DKQ, const int DV, const int ncols, const int device_id) {
    return (ggml_cuda_fattn_tile_get_config(DKQ, DV, ncols, device_id) >> 14) & ((1 << 9) - 1);
}
...
static __host__ int ggml_cuda_fattn_tile_get_nbatch_K(const int DKQ, const int DV, const int ncols, const int device_id) {
    return (ggml_cuda_fattn_tile_get_config(DKQ, DV, ncols, device_id) >> 23) & ((1 << 9) - 1);
}
...
static void launch_fattn_tile_switch_ncols1(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if constexpr (DKQ <= 128) {
        if (Q->ne[1] > 32/ncols2) {
            ...
            const int nwarps    = ggml_cuda_fattn_tile_get_nthreads (DKQ, DV, cols_per_block, id) / warp_size;
            const int nbatch_fa = ggml_cuda_fattn_tile_get_nbatch_fa(DKQ, DV, cols_per_block, id);

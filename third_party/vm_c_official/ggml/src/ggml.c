...
// ggml_vm_c_tp_allreduce — Tensor Parallelism 跨 GPU AllReduce
// 在 CUDA backend 上执行真正的 NCCL 或 internal allreduce；CPU 上为恒等映射

struct ggml_tensor * ggml_vm_c_tp_allreduce(
        struct ggml_context * ctx,
        struct ggml_tensor  * tensor,
        void                * tp,
        int32_t               tp_size) {
    ...
    result->op     = GGML_OP_VM_C_TP_ALLREDUCE;
    result->src[0] = tensor;
    result->src[1] = NULL;
    result->src[2] = NULL;

    // 在 op_params 中存储 tp 指针和 tp_size
    // op_params[0..1] = tp (64-bit pointer split)
    // op_params[2]    = tp_size
    memcpy(&result->op_params[0], &tp, sizeof(void *));
    result->op_params[2] = tp_size;
